...
`cd ~/Downloads/CM0102\ Starter\ Kit\ Mac\ Version/` (or wherever you downloaded the app to) followed by `sudo xattr -crs CM0102StarterKit.app` (this requires your login password). This may reset the logo on the Starter Kit (to the standard 'Wineskin' logo), but it should fix it so you can run the tool. If this doesn't work for whatever reason (you don't have a password or it doesn't accept your password), try running the command without the "sudo" bit at the front, i.e. `xattr -crs CM0102StarterKit.app`.
If you get an error message such as "xattr: No such file: CM0102StarterKit.app/Contents/Resources/dosdevices/d:" or "xattr: No such file: CM0102StarterKit.app/Contents/Resources/drive_c/users/Wineskin/Templates" then run the following commands for every error message you receive. For the above error messages, I would run:  
`sudo rm CM0102StarterKit.app/Contents/Resources/dosdevices/d`  
`sudo rm CM0102StarterKit.app/drive_c/users/Wineskin/Templates`  
This deletes those files, which is fine as they are just aliases (shortcuts) to other files/folders on your system. Then try running `sudo xattr -crs CM0102StarterKit.app` again. If that doesn't work, then unfortunately I'm not sure what else to suggest.
...
-   **Load All Players** - This will ensure all players from your chosen database are loaded into the game regardless as to how many leagues you select. Very useful if you normally select lots of leagues just to increase the number of available players, as having more leagues makes the game much, much slower. Loading all players does slow performance to a degree, but not as much as loading all leagues does (and loading all leagues means a much higher chance of a game-breaking bug occurring at any point!).

-   **Tapani Regen Code** - Fixes an issue where regens get an ever increasing 'Positioning' attribute, which eventually leads to very low scoring games years down the line (basically every defender becomes Maldini!).
